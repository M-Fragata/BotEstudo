            ...
            const response = await fetch('https://integrate.api.nvidia.com/v1/chat/completions', {
                ...
                body: JSON.stringify({
                    ...
                    messages: [
                        {
                            role: 'system',
                            content: 'Você é um professor universitário especializado em programação. Analise cada questão e determine a resposta correta. Pode raciocinar, mas TERMINE sua resposta com o JSON de respostas.'
                        },
                        {
                            role: 'user',
                            content: prompt
...
async function analisarExercicios(listas: any[]): Promise<Record<string, Record<number, string>> | null> {
    ...
    if (match) {
        try {
            ...
            console.log('  ⚠️ JSON extraído inválido, tentando parse direto...');
        }
    }
    try {
